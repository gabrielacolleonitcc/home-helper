...
export default function ProfileScreen({ navigation }) {
  ...
  const [ratings, setRatings] = useState(Array(services.length).fill(0));

  const handleRating = (serviceId, rate) => {
    const newRatings = [...ratings];
    newRatings[serviceId] = rate;
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.servicesTitle}>JARDINAGEM E PAISAGISMO</Text>
      <ScrollView>
          ...
          {profissionais.map((profissional) => (
            <View key={profissional.id} style={styles.serviceItem}>
              <TouchableOpacity
                ...
                onPress={() => navigation.navigate('PrestadorId', { serviceId: 3,id: profissional.id, name: profissional.nome, telefone: profissional.telefone, valor_hora: profissional.valor_hora, media: profissional.media, atendimentos: profissional.atendimentos, avaliacoes: profissional.avaliacaos })}
              >
